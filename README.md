...
HiDEF-seq analysis also requires standard germline sequencing data for filtering germline variants, which should also be sequenced on PacBio Revio instruments (i.e., same platform as HiDEF-seq data) to minimize false-positive calls due to missed germline variants.
...
- `processReads` — demultiplexes and aligns reads, and for subread data, also creates ccs consensus sequences.
- `splitBAMs` — splits reads into chunks (per the `analysis_chunks` YAML parameter) for downstream processing
-  `prepareFilters` — prepares files used for filtering
- `extractCalls` — extracts calls
-  `filterCalls` — filter calls
- `calculateBurdens` — calculate burdens, and outputs HiDEF-seq genome coverage files
- `outputResults` — outputs results
- `removeIntermediateFiles` — deletes files from all except the `processReads` workflow, genome coverage files from the `calculateBurdens` workflow, and `outputResults` workflow.

Any individual workflow or multiple workflows can be selected. When running multiple workflows, the following rules apply:
- `all` cannot be combined with other entries.
- Selected workflows must be contiguous (for example `processReads,splitBAMs,prepareFilters` is valid; `processReads,extractCalls` is rejected because `splitBAMs` is skipped).
