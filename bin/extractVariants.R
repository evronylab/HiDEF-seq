...
### Extract read indels
######################
#Save IRanges objects with the locations of read insertions and deletions and the lengths of read insertions relative to reference space.
cat("#### Extracting read indels...")

bam.gr$insertion_ranges_refspace <- bam.gr$cigar %>%
  cigarRangesAlongReferenceSpace(ops="I", pos=start(bam.gr))

bam.gr$deletion_ranges_refspace <- bam.gr$cigar %>%
  cigarRangesAlongReferenceSpace(ops="D", pos=start(bam.gr))

bam.gr$insertion_lengths_refspace <- bam.gr$cigar %>%
  cigarRangesAlongQuerySpace(ops="I") %>%
  width
...
if(cigar_query_sbs %>% as.data.frame %>% nrow == 0){
  ...
  sbs.df <- tibble(zm=integer(),seqnames=factor(),strand=factor(),pos_queryspace=integer(),pos_refspace=integer(),ref=character(),alt=character(),qual=integer(),sa=integer(),sm=integer(),sx=integer())
  
}else{
  #Single base substitution positions in query space
  sbs_pos_queryspace <- cigar_query_sbs %>%
    ...
    rename("pos_queryspace" = "start") %>%
    uncount(weights=width) %>%
    select(zm,strand,pos_queryspace) %>%
    group_by(zm,strand,pos_queryspace) %>%
    mutate(pos_queryspace = pos_queryspace + row_number() - 1L) %>%
    ungroup
  
  #Single base substitution positions in reference space, and also annotate with positions in query space for later joining
  sbs_pos_refspace <- cigarRangesAlongReferenceSpace(bam.gr$cigar,ops="X",pos=start(bam.gr))
  sbs_pos_refspace <- sbs_pos_refspace %>%
    unlist(use.names=FALSE) %>%
    setNames(cigar_query_sbs %>% unlist(use.names=FALSE) %>% start) %>%
    relist(.,sbs_pos_refspace) %>%
    setNames(str_c(bam.gr$zm,strand(bam.gr) %>% as.character,sep="_")) %>%
    as.data.frame %>%
    separate(group_name,sep="_",into=c("zm","strand")) %>%
    mutate(
    ...
    rename(
      "pos_refspace" = "start",
      "pos_queryspace" = "names"
    ) %>%
    uncount(weights=width) %>%
    select(zm,strand,pos_refspace,pos_queryspace) %>%
    group_by(zm,strand,pos_refspace,pos_queryspace) %>%
    mutate(
      pos_refspace = pos_refspace + row_number() - 1L,
      pos_queryspace = pos_queryspace + row_number() - 1L
    ) %>%
    ungroup %>%
    left_join(
  ...
  sbs_pos_refspace$ref <- sbs_pos_refspace %>%
    mutate(strand="+") %>%
    makeGRangesFromDataFrame(
      seqnames.field="seqnames",
      start.field="pos_refspace",
      end.field="pos_refspace",
      strand.field="strand"
    ) %>%
  ...
  sbs_alt <- extractAt(bam.gr$seq,cigar_query_sbs) %>%
    ...
    as.list %>%
    enframe %>%
    unnest_longer(col=value,indices_to="pos_queryspace",values_to="alt") %>%
    separate_longer_position(alt,width=1) %>%
    separate(name,sep="_",into=c("zm","strand")) %>%
    mutate(
      zm = as.integer(zm),
      strand = as.factor(strand),
      pos_queryspace = as.integer(pos_queryspace)
      ) %>%
    group_by(zm,strand,pos_queryspace) %>%
    mutate(pos_queryspace = pos_queryspace + row_number() - 1L) %>%
    ungroup
  ...
  sbs_qual <- extractAt(bam.gr$qual,cigar_query_sbs) %>%
    ...
    as.list %>%
    enframe %>%
    unnest_longer(col=value,indices_to="pos_queryspace",values_to="qual") %>%
  ...
  sbs_pos_queryspace.list <- sbs_pos_queryspace %>%
    mutate(zm_strand = str_c(zm,strand,sep="_")) %>%
    select(zm_strand,pos_queryspace) %>%
    as.data.table %>%
  ...
  sbs_sa <- map2(sa[sbs_pos_queryspace.list %>% names], sbs_pos_queryspace.list, ~ .x[.y] %>% set_names(.y)) %>%
    enframe %>%
    separate(name,sep="_",into=c("zm","strand")) %>%
    unnest_longer(col=value, values_to="sa", indices_to="pos_queryspace") %>%
    mutate(
      zm = as.integer(zm),
      strand = as.factor(strand),
      pos_queryspace = as.integer(pos_queryspace)
  ...
  sbs_sm <- map2(sm[sbs_pos_queryspace.list %>% names], sbs_pos_queryspace.list, ~ .x[.y] %>% set_names(.y)) %>%
    enframe %>%
    separate(name,sep="_",into=c("zm","strand")) %>%
    unnest_longer(col=value, values_to="sm", indices_to="pos_queryspace") %>%
    mutate(
      zm = as.integer(zm),
      strand = as.factor(strand),
      pos_queryspace = as.integer(pos_queryspace)
  ...
  sbs_sx <- map2(sx[sbs_pos_queryspace.list %>% names], sbs_pos_queryspace.list, ~ .x[.y] %>% set_names(.y)) %>%
    enframe %>%
    separate(name,sep="_",into=c("zm","strand")) %>%
    unnest_longer(col=value, values_to="sx", indices_to="pos_queryspace") %>%
    mutate(
      zm = as.integer(zm),
      strand = as.factor(strand),
      pos_queryspace = as.integer(pos_queryspace)
  ...
  #Combine sbs info
  sbs.df <- sbs_pos_queryspace %>%
    left_join(sbs_alt, by=join_by(zm,strand,pos_queryspace)) %>%
    left_join(sbs_qual, by=join_by(zm,strand,pos_queryspace)) %>%
    left_join(sbs_pos_refspace, by=join_by(zm,strand,pos_queryspace)) %>%
    left_join(sbs_sa, by=join_by(zm,strand,pos_queryspace)) %>%
    left_join(sbs_sm, by=join_by(zm,strand,pos_queryspace)) %>%
    left_join(sbs_sx, by=join_by(zm,strand,pos_queryspace)) %>%
    select(zm,seqnames,strand,pos_queryspace,pos_refspace,ref,alt,qual,sa,sm,sx)
  
  rm(sbs_pos_queryspace,sbs_pos_queryspace.list,sbs_alt,sbs_qual,sbs_pos_refspace,sbs_sa,sbs_sm,sbs_sx)
