...
#Load VCF files
for(i in vcf_files %>% pull(individual_id) %>% unique){
  
  cat("> Processing individual:",i,"\n")
      
	germline_vcf_variants <- list()
  
  vcf_files_individual <- vcf_files %>% filter(individual_id == i)
  
  for(j in 1:nrow(vcf_files_individual)){
  	
  	germline_vcf_file <- vcf_files_individual %>% pluck("germline_vcf_file",j)
  	germline_vcf_filtergroup <- vcf_files_individual %>% pluck("germline_vcf_filtergroup",j)
  ...
  qs_save(
    ...
    str_c(i,".germline_vcf_variants.qs2")
    )
  
  rm(germline_vcf_variants)
