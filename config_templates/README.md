...
| `filtergroups[]`.`ccs_sbs_pad`, `ccs_ins_pad`, `ccs_del_pad`<br />(molecule-region filter) | integer, string, string | yes | Specification of padding to add around each HiDEF-seq (i.e. non-germline) SBS (`ccs_sbs_pad`), insertion (`ccs_ins_pad`), and deletion (`ccs_del_pad`) call in each strand for filtering HiDEF-seq calls. `ccs_sbs_pad` is the number of bases in each immediately adjacent flank to filter. `ccs_ins_pad` and `ccs_del_pad` are specified using the same `m<multiplier>b<offset>` padding syntax described above (set to `NA` or `m0b0` to disable). The `ccs_ins_pad` and `ccs_del_pad` filters are not applied to indel calls. |
