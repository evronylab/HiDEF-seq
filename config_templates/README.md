...
| `mem_filterMutationErrorProbabilityChromgroupFiltergroup`, `time_filterMutationErrorProbabilityChromgroupFiltergroup`, `maxRetries_filterMutationErrorProbabilityChromgroupFiltergroup` | string/integer | optional | Analogous settings for `filterMutationErrorProbabilityChromgroupFiltergroup` processes. |
...
| `filtergroups[].max_mutation_errors_per_bp`<br />(call-level filter) | numeric or blank | optional | Optional maximum mutation errors-per-bp cutoff applied by `filterMutationErrorProbability` only to calls with `call_class` = `SBS` or `indel` and `SBSindel_call_type` = `mutation`. If blank, this filter is disabled for that filtergroup. |
| `filtergroups[].filterMutationErrorProbability_qual_bins`<br />(mutation error model setting) | list of numeric upper edges (or `Inf`) | yes when `max_mutation_errors_per_bp` is set | Quality-bin edges used by `filterMutationErrorProbability` to compute context+quality-bin error rates and apply high-to-low bin cutoff filtering per context. Recommended: `[10,20,30,40,50,60,70,80,89,Inf]` (i.e., bins `1-10`, `11-20`, …, `81-89`, `>=90`). |
