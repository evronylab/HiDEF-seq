...
| `coverage`           | Number of final interrogated units at the position. For `bc_orientation = all_bc_orientations` files, this is duplex coverage in base pairs (i.e., coverage by each duplex molecule is counted as 1). For barcode-orientation-specific files, this is interrogated base coverage limited to the specified asymmetric `bc_orientation` value. |
