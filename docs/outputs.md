...
- Shared identifiers: `analysis_id`, `individual_id`, `sample_id`, `chromgroup`, `filtergroup`, `bc`, `call_class`, `call_type`, `SBSindel_call_type`. Rows with `bc = all_bc` contain data from both strands (i.e. both final-round barcode orientations). When the final demultiplexing round is asymmetric, tables also contain rows for single-strand call types whose data is limited to strands with the specified `bc` value. For mutation burden rows, `interrogated_bases_or_bp` reflects interrogated base pairs (bp) (i.e., duplex coverage). For single-strand call type rows with `bc = all_bc`, `interrogated_bases_or_bp` reflects interrogated bases summed across both duplex strands. For single-strand call type rows with individual-strand `bc` values, `interrogated_bases_or_bp` reflects interrogated bases limited to strands with the specified `bc` value.
...
Table containing genome coverage tracks and trinucleotide distributions of interrogated base pairs for `bc = all_bc` rows, and interrogated bases for individual-strand `bc` rows, split into one row for each combination of `bc`, `call_class`, `call_type`, and `SBSindel_call_type`. Rows with `bc = all_bc` contain data from both strands (i.e. both final-round barcode orientations). When the final demultiplexing round is asymmetric, the table also includes rows for single-strand call types whose data is limited to strands with the specified `bc` value. For each row, there are metadata identifiers (`analysis_id`, `individual_id`, `sample_id`, `chromgroup`, `filtergroup`, `bc`, `call_class`, `call_type`, `SBSindel_call_type`) and the following tables:

| Column | Description |
| --- | --- |
| `bam.gr.filtertrack.coverage` | List-column of RleList coverage objects. Rows with `bc = all_bc` store coverage for interrogated base pairs (i.e., duplex coverage counting once coverage by both strands of a molecule). Rows with individual-strand `bc` values store strand-level coverage for reads where the strand has the specified `bc` value. |
| `bam.gr.filtertrack.reftnc_pyr`, `bam.gr.filtertrack.reftnc_both_strands`, `bam.gr.filtertrack.reftnc_template_strand` | Same format as `.bam.gr.filtertrack.reftnc_pyr.tsv`, `.bam.gr.filtertrack.reftnc_both_strands.tsv`, and `.bam.gr.filtertrack.reftnc_template_strand.tsv` described above. For `bc = all_bc`, `.reftnc_pyr.tsv` is based on interrogated base pairs, and `.reftnc_both_strands.tsv` and `.reftnc_template_strand.tsv` count both reference-strand contexts. For individual-strand `bc` rows, these tables use strand-level coverage for the specified barcode strand. |
...
Columns as per `.finalCalls.burdens.tsv` described above, in one table for all call types and emitted `bc` values.
