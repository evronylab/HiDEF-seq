...
- `.bam.gr.filtertrack.reftnc_pyr.tsv`: `reftnc_pyr`, `count`, `fraction`, `fraction_ratio_to_genome`, `fraction_ratio_to_genome_chromgroup` summarizing counts of interrogated bases or base pairs collapsed to central pyrimidine trinucleotides together with ratios of the interrogated-base trinucleotide fractions to the whole-genome and chromgroup-restricted fractions. Rows with `bc_orientation = all_bc_orientations` summarize interrogated base pairs; rows with individual-strand `bc_orientation` values summarize interrogated bases from the specified barcode strand.
