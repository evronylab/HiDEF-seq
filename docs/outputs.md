...
For every combination of `call_class`, `call_type`, and `SBSindel_call_type`, the `calculateBurdensChromgroupFiltergroup` process that is run for each `sample_id`, `chromgroup`, and `filtergroup` writes a bgzipped BED file named:
 `[analysis_id].[individual_id].[sample_id].[chromgroup].[filtergroup].[call_class].[call_type].[SBSindel_call_type].coverage_reftnc.bed.gz` for `bc_orientation = all_bc_orientations` files, and when the final demultiplexing round is asymmetric it also writes barcode-orientation-specific files for single-strand call types named `[analysis_id].[individual_id].[sample_id].[chromgroup].[filtergroup].[bc_orientation].[call_class].[call_type].[SBSindel_call_type].coverage_reftnc.bed.gz`. Each file has a companion Tabix index (`.tbi`) and contains genome-wide per-base HiDEF-seq coverage (see definition in below table) together with reference trincucleotide sequences for all non-zero coverage positions:
...
| `coverage`           | Number of final interrogated units at the position. For `bc_orientation = all_bc_orientations` files, this is interrogated base pairs (i.e., coverage by each duplex molecule is counted as 1). For barcode-orientation-specific files, this is interrogated bases limited to the specified asymmetric `bc_orientation` value. |
...
Table containing genome coverage tracks and trinucleotide distributions of interrogated base pairs for `bc_orientation = all_bc_orientations` rows, and interrogated bases for individual-strand `bc_orientation` rows, split into one row for each combination of `bc_orientation`, `call_class`, `call_type`, and `SBSindel_call_type`. Rows with `bc_orientation = all_bc_orientations` contain data from both strands (i.e. both final-round barcode orientations). When the final demultiplexing round is asymmetric, the table also includes rows for single-strand call types whose data is limited to strands with the specified `bc_orientation` value. For each row, there are metadata identifiers (`analysis_id`, `individual_id`, `sample_id`, `chromgroup`, `filtergroup`, `bc_orientation`, `call_class`, `call_type`, `SBSindel_call_type`) and the following tables:
