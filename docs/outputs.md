...
- `.bam.gr.filtertrack.reftnc_both_strands.tsv`: `reftnc`, `count`, `fraction`, `fraction_ratio_to_genome`, `fraction_ratio_to_genome_chromgroup` summarizing counts of interrogated bases after counting both reference-strand trinucleotide contexts at each interrogated position. For individual-strand `bc_orientation` rows, this table contains the template-strand trinucleotide counts for interrogated bases from the specified barcode strand.
- `.bam.gr.filtertrack.reftnc_template_strand.tsv`: `reftnc`, `count`, `fraction`, `fraction_ratio_to_genome`, `fraction_ratio_to_genome_chromgroup` summarizing counts of interrogated bases using the reference trinucleotide context corresponding to the sequencer-polymerase template strand.
