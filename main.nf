...
workflow processReads {
    ...
    limaDemux.out.bam.flatten() | map { f -> tuple(f, file("${f}.pbi") | subscribe { println "DEBUG: limaDemux.out: $it" }
    ...
    countZMWs_ch = countZMWs_ch
      .merge(
        ...
        pbmm2Align.out.collect(flat: false) | map { f -> tuple(f[2], f[3], "aligned_zmwcount.txt") }
      )
      .map { it.transpose() }
