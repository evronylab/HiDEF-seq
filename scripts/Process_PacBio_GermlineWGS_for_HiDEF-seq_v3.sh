...
sbatch -t 3600 -c 16 --mem 64G --dependency=afterok:$alignjobid --wrap "singularity run -B `pwd`:`pwd` -B `dirname $reffasta`:`dirname $reffasta` -B /usr/lib/locale/:/usr/lib/locale/ -B `dirname $output_basename`:`dirname $output_basename` $deepvariant_sif /opt/deepvariant/bin/run_deepvariant --model_type=PACBIO --ref=$reffasta --reads=$output_basename.bam --output_vcf=$output_basename.deepvariant.vcf.gz --num_shards=16"
