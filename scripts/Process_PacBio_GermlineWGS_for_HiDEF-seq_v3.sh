...
male_female=$8 #male or female
par_bed=`readlink -f $9`

if [ $male_female == "male" ]
then
	deepvariant_haploid_settings="--haploid_contigs='chrX,chrY' --par_regions_bed=$par_bed"
else
	deepvariant_haploid_settings=""
fi
...
sbatch -t 2880 -c 32 --mem 64G --dependency=afterok:$alignjobid --wrap "singularity run -B `pwd`:`pwd` -B `dirname $reffasta`:`dirname $reffasta` -B /usr/lib/locale/:/usr/lib/locale/ -B `dirname $output_basename`:`dirname $output_basename` $clair3_sif /opt/bin/run_clair3.sh --bam_fn=$output_basename.bam --ref_fn=$reffasta --threads=32 --platform=\"hifi\" --model_path=\"/opt/models/hifi_revio\" --include_all_ctgs --output=${output_basename}.clair3 --remove_intermediate_dir; mv ${output_basename}.clair3/merge_output.vcf.gz ${output_basename}.clair3.vcf.gz; mv ${output_basename}.clair3/merge_output.vcf.gz.tbi ${output_basename}.clair3.vcf.gz.tbi"

## iii. Call variants with DeepVariant
sbatch -t 3600 -c 16 --mem 64G --dependency=afterok:$alignjobid --wrap "singularity run -B `pwd`:`pwd` -B `dirname $reffasta`:`dirname $reffasta` -B /usr/lib/locale/:/usr/lib/locale/ -B `dirname $output_basename`:`dirname $output_basename` $deepvariant_sif /opt/deepvariant/bin/run_deepvariant --num_shards=16 --model_type=PACBIO --disable_small_model=true ${deepvariant_haploid_settings} --vcf_stats_report=true --ref=$reffasta --reads=$output_basename.bam --output_vcf=$output_basename.deepvariant.vcf.gz"